...
export const PetCard: React.FC<PetCardProps> = ({ pet, owner, onViewUser }) => {
    ...
    return (
        <Link 
            to={`/mascota/${pet.id}`}
            className="bg-white rounded-xl shadow-lg overflow-hidden transition-transform transform hover:-translate-y-1 hover:shadow-2xl flex flex-col h-full cursor-pointer relative block"
        >
             {isReunited && (
