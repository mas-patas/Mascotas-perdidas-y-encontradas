...
export const useCommentsByPetId = (petId: string | undefined) => {
  return useQuery({
    queryKey: queryKeys.comments(petId!),
...
export const useCommentLikes = (commentId: string | undefined) => {
  return useQuery({
    queryKey: queryKeys.commentLikes(commentId!),
...
export const useHasUserLikedComment = (commentId: string | undefined, userId: string | undefined) => {
  return useQuery({
    queryKey: [...queryKeys.commentLikes(commentId!), 'user', userId],
      ...
      .on(
        ...
        (payload: any) => {
          if (payload.new?.pet_id) {
            queryClient.invalidateQueries({ queryKey: queryKeys.comments(payload.new.pet_id) });
          }
      ...
      .on(
        ...
        (payload: any) => {
          if (payload.new?.comment_id) {
            queryClient.invalidateQueries({ queryKey: queryKeys.commentLikes(payload.new.comment_id) });
          }
