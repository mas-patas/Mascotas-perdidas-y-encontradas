...
const PetSection: React.FC<{ 
    ...
    initialPets.forEach(pet => allPetsMap.set(pet.id, pet));
    additionalPets.forEach(pet => allPetsMap.set(pet.id, pet));
    ...
    const fetchMoreHorizontal = async () => {
        ...
        try {
            ...
            if (data && data.length > 0) {
                ...
                // Filter out pets that are already in initialPets to avoid duplicates
                const existingIds = new Set(initialPets.map(p => p.id));
                const newUniquePets = mappedNewPets.filter(p => !existingIds.has(p.id));
