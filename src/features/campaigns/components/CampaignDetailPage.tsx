...
import { CalendarIcon, LocationMarkerIcon, ChevronLeftIcon, ChevronRightIcon, PhoneIcon, GoogleMapsIcon, WazeIcon } from '@/shared/components/icons';
...
const CampaignDetailPage: React.FC<CampaignDetailPageProps> = ({ campaign: propCampaign, onClose }) => {
    ...
    return (
        <div className="bg-white rounded-lg shadow-xl w-full max-w-4xl mx-auto">
            <Helmet>
                <title>{campaign.title} - Campañas Más Patas</title>
                <meta name="description" content={`${campaign.type}: ${campaign.title} el ${formattedDate} en ${simplifiedLocation}.`} />
                <meta property="og:title" content={`${campaign.title} - Más Patas`} />
