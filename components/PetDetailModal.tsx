...
import { CalendarIcon, LocationMarkerIcon, PhoneIcon, ChevronLeftIcon, ChevronRightIcon, TagIcon, DogIcon, CatIcon, ChatBubbleIcon, EditIcon, TrashIcon, FacebookIcon, TwitterIcon, WhatsAppIcon, PrinterIcon, HeartIcon, CheckCircleIcon, FlagIcon } from './icons';
...
export const PetDetailModal: React.FC<PetDetailModalProps> = ({ pet, onClose, onStartChat, onEdit, onDelete, onGenerateFlyer, onUpdateStatus, users, onViewUser, onReport }) => {
    ...
    return (
        <>
            ...
            >
                <div 
                    className="bg-white rounded-lg shadow-2xl w-full max-w-3xl max-h-[90vh] overflow-y-auto flex flex-col md:flex-row"
                ...
                >
                    ...
                    <div className="w-full md:w-1/2 p-6 flex flex-col">
                        <div className="flex justify-between items-start mb-4">
                             {showIconAsTitle ? (
                                 <h2 className="text-3xl font-bold text-brand-dark w-full text-center">
                                    {pet.animalType === ANIMAL_TYPES.PERRO && <DogIcon className="h-16 w-16 inline-block text-brand-dark" />}
                                    {pet.animalType === ANIMAL_TYPES.GATO && <CatIcon className="h-16 w-16 inline-block text-brand-dark" />}
                                 </h2>
                             ) : (
                        ...
                        <div className="mt-auto pt-4 border-t border-gray-200 space-y-4">
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                ...
                                {pet.status === PET_STATUS.PERDIDO && (
                                    <button
                                        onClick={() => onGenerateFlyer(pet)}
                                        className={`w-full flex items-center justify-center gap-2 bg-gray-600 text-white font-bold py-2 px-4 rounded-lg hover:bg-gray-700 transition-colors ${!(isOwner || canAdmin) ? 'col-span-2' : ''}`}
                                    >
                                        <PrinterIcon />
